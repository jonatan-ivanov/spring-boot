...
class SpanTagAnnotationHandler {
	...
	void addAnnotatedParameters(MethodInvocation pjp) {
		try {
			...
			List<ObservabilityTracingAnnotatedParameter> annotatedParameters = ObservabilityTracingAnnotationUtils
	...
	private void getAnnotationsFromInterfaces(MethodInvocation pjp, Method mostSpecificMethod,
			List<ObservabilityTracingAnnotatedParameter> annotatedParameters) {
			...
			for (Class<?> implementedInterface : implementedInterfaces) {
				for (Method methodFromInterface : implementedInterface.getMethods()) {
					if (methodsAreTheSame(mostSpecificMethod, methodFromInterface)) {
						List<ObservabilityTracingAnnotatedParameter> annotatedParametersForActualMethod = ObservabilityTracingAnnotationUtils
	...
	private void mergeAnnotatedParameters(List<ObservabilityTracingAnnotatedParameter> annotatedParametersIndices,
			List<ObservabilityTracingAnnotatedParameter> annotatedParametersIndicesForActualMethod) {
		for (ObservabilityTracingAnnotatedParameter container : annotatedParametersIndicesForActualMethod) {
			final int index = container.parameterIndex;
			boolean parameterContained = false;
			for (ObservabilityTracingAnnotatedParameter parameterContainer : annotatedParametersIndices) {
	...
	private void addAnnotatedArguments(List<ObservabilityTracingAnnotatedParameter> toBeAdded) {
		for (ObservabilityTracingAnnotatedParameter container : toBeAdded) {
	...
	private String resolveTagKey(ObservabilityTracingAnnotatedParameter container) {
