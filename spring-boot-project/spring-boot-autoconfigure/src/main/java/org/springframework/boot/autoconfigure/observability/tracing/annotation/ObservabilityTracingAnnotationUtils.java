 ...
 * @since 1.2.0
 */
final class ObservabilityTracingAnnotationUtils {

	private static final Log log = LogFactory.getLog(ObservabilityTracingAnnotationUtils.class);

	private ObservabilityTracingAnnotationUtils() {
	...
	static List<ObservabilityTracingAnnotatedParameter> findAnnotatedParameters(Method method, Object[] args) {
		Annotation[][] parameters = method.getParameterAnnotations();
		List<ObservabilityTracingAnnotatedParameter> result = new ArrayList<>();
		int i = 0;
		for (Annotation[] parameter : parameters) {
			for (Annotation parameter2 : parameter) {
				if (parameter2 instanceof SpanTag) {
					result.add(new ObservabilityTracingAnnotatedParameter(i, (SpanTag) parameter2, args[i]));
				}
			}
			i++;
		}
