...
@ConditionalOnProperty(value = "spring.observability.tracing.zipkin.sender.type", havingValue = "kafka")
class ZipkinKafkaSenderConfiguration {
	...
	static class ZipkinKafkaSenderBeanConfiguration {
		...
		Sender kafkaSender(KafkaProperties config, Environment environment) {
			...
			// See gh-1990
			String topic = environment.getProperty("spring.observability.tracing.zipkin.kafka.topic", "zipkin");
