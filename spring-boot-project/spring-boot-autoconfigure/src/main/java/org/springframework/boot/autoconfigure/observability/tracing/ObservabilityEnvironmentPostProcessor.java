...
class TraceEnvironmentPostProcessor implements EnvironmentPostProcessor {
	...
	public void postProcessEnvironment(ConfigurableEnvironment environment, SpringApplication application) {
		...
		if (Boolean.parseBoolean(environment.getProperty("spring.observability.tracing.enabled", "true"))) {
			map.put("logging.pattern.level",
					"%5p [${spring.observability.zipkin.service.name:" + "${spring.application.name:}},%X{traceId:-},%X{spanId:-}]");
		}
