 ...
 */
abstract class AbstractObservabilityTracingMethodInvocationProcessor implements TracingMethodInvocationProcessor, BeanFactoryAware {

	private static final Log logger = LogFactory.getLog(AbstractObservabilityTracingMethodInvocationProcessor.class);

	BeanFactory beanFactory;

	private TracingNewSpanParser newSpanParser;
	...
	void addTags(MethodInvocation invocation, Span span) {
		ObservabilityTracingAnnotationSpan.ANNOTATION_NEW_OR_CONTINUE_SPAN.wrap(span)
				.tag(ObservabilityTracingAnnotationSpan.Tags.CLASS, invocation.getThis().getClass().getSimpleName())
				.tag(ObservabilityTracingAnnotationSpan.Tags.METHOD, invocation.getMethod().getName());
	}
	...
	void logEvent(Span span, String name) {
		...
		ObservabilityTracingAnnotationSpan.ANNOTATION_NEW_OR_CONTINUE_SPAN.wrap(span).event(name);
	}
	...
	TracingNewSpanParser newSpanParser() {
		if (this.newSpanParser == null) {
			this.newSpanParser = this.beanFactory.getBean(TracingNewSpanParser.class);
		}
