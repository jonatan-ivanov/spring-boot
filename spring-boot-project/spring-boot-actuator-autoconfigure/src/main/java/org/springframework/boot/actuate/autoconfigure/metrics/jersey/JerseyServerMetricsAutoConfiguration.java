...
public class JerseyServerMetricsAutoConfiguration {
	...
	// @Bean
	// public ResourceConfigCustomizer
	// jerseyServerMetricsResourceConfigCustomizer(MeterRegistry meterRegistry,
	// JerseyTagsProvider tagsProvider) {
	// Server server = this.properties.getWeb().getServer();
	// return (config) -> config.register(
	// new MetricsApplicationEventListener(meterRegistry, tagsProvider,
	// server.getRequest().getMetricName(),
	// server.getRequest().getAutotime().isEnabled(), new
	...
	// String metricName =
	// this.properties.getWeb().getServer().getRequest().getMetricName();
	// MeterFilter filter = new OnlyOnceLoggingDenyMeterFilter(
	// () -> String.format("Reached the maximum number of URI tags for '%s'.",
	// metricName));
	// return MeterFilter.maximumAllowableTags(metricName, "uri",
	// this.properties.getWeb().getServer().getMaxUriTags(),
	// filter);
	// }
	//
	// /**
